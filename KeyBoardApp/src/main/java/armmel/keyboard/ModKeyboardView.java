...
public class ModKeyboardView extends View implements View.OnClickListener {
    ...
    private void drawAllKeys() {
        ...
        if(oldDarkStatus != isDark || (bg == null || keyBackground == null)) {
            bg = getResources().getDrawable(isDark ? R.drawable.keyboard_background_dark :R.drawable.keyboard_background_light,context.getTheme());
            keyBackground = getResources().getDrawable(isDark? R.drawable.key_background_dark:R.drawable.key_background_light,context.getTheme());
    ...
    private void drawOneKey(Canvas canvas, int index) {
        ...
        if (labels != null) {
            ...
            canvas.drawText(labels[0], centerX - r.width()/2 - r.left, centerY  + r.height()/2 - r.bottom, paint);
            if (key.codes.length == 5) {
                ...
                canvas.drawText(labels[1], padding.left + marginX, centerY + r.height() / 2 - r.bottom, paint);
                paint.setColor(currentcodeindex == 3 ? color_current : color_normal);
                paint.getTextBounds(labels[3], 0, 1, r);
                canvas.drawText(labels[3], key.width - marginX - r.right, centerY + r.height() / 2 - r.bottom, paint);
                paint.setColor(currentcodeindex == 2 ? color_current : color_normal);
                paint.getTextBounds(labels[2], 0, 1, r);
                canvas.drawText(labels[2], centerX - r.width()/2 - r.left, padding.top + marginY - r.top, paint);
                paint.setColor(currentcodeindex == 4 ? color_current : color_normal);
                paint.getTextBounds(labels[4], 0, 1, r);
                canvas.drawText(labels[4], centerX - r.width()/2 - r.left, key.height - marginY - r.bottom, paint);
            }
