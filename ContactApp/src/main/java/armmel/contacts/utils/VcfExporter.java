...
public class VcfExporter {
    ...
    public String exportToVcf(VCardVersion version) {
        ...
        for (Contact contact : contacts) {
            ...
            vCard.append("VERSION:").append(version == VCardVersion.VCARD_21 ? "2.1" : "3.0").append("\n");

            vCard.append("FN:").append(contact.getName()).append("\n");
            if (contact.getOrg() != null) vCard.append("ORG:").append(contact.getOrg()).append("\n");
            if (contact.getTitle() != null) vCard.append("TITLE:").append(contact.getTitle()).append("\n");

            List<ContactPhone> phones = contactPhoneDao.getByContactId(contact.getId());
            for (ContactPhone phone : phones) {
                vCard.append("TEL;TYPE=").append(phone.getType() != null ? phone.getType() : "CELL")
                     .append(":").append(phone.getPhone()).append("\n");
            }

            List<ContactEmail> emails = contactEmailDao.getByContactId(contact.getId());
            for (ContactEmail email : emails) {
                vCard.append("EMAIL;TYPE=").append(email.getType() != null ? email.getType() : "HOME")
            ...
            for (ContactAddress address : addresses) {
                vCard.append("ADR;TYPE=").append(address.getType() != null ? address.getType() : "HOME")
                     .append(":;;").append(address.getAddress().replace("\n", "\\n")).append(";;;;\n");
            }
            ...
            if (photo != null && photo.getPhoto() != null) {
                ...
                if (version == VCardVersion.VCARD_21) {
                    ...
                    vCard.append("PHOTO;ENCODING=b;TYPE=").append(mimeType).append(":").append(base64Photo).append("\n");
                }
            }
